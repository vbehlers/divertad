...
class EnhancedBellScheduleManager {
    ...
    updateDashboard() {
        ...
        // Update next bell
        const nextBellText = kpiData.nextBell.timeUntil > 0 
            ? `${kpiData.nextBell.time} / ${kpiData.nextBell.countdown}m`
            : `${kpiData.nextBell.time} / ${kpiData.nextBell.countdown}`;
        this.updateElement('next-bell', nextBellText);
        ...
        if (dateHeaderElement) {
            this.updateElement('current-date-header', kpiData.currentDate);
        }
    ...
    updateScheduleTable(schedule) {
        ...
        if (!tbody || !schedule || !schedule.periods) {
            if (tbody) tbody.innerHTML = '<tr><td colspan="4" class="text-center p-4">No schedule available</td></tr>';
            return;
        }
    ...
    /**
     * Get period status for table display
     */
                ...
                schedule.forEach(period => {
                    content += `
                        <tr class="border-b-3 border-gray-100">
                            ...
                            <td class="type-p5 p-4">${period.start_time} - ${period.end_time}</td>
                            <td class="type-p5 p-4">${period.duration_minutes} min</td>
                            <td class="type-p5 p-4 text-gray-500">${this.getPeriodNotes(period.period_name)}</td>
                        </tr>
                    `;
                });
